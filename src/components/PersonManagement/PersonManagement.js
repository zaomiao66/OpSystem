...
export default class PersonManagement extends Component {
    ...
    getTree =(arr, root) => {
      ...
      return root.map(item => (<TreeNode title={arr[item].name} key={arr[item].id}>
        {
                                ...
                                : <div>
                                  {
                                        !state.leftIsSearching ?
                                          <div className="teachersBeAddedBottom">
                                            {
                                                state.teachersInLeft.map((item, idx) => <span key={idx} style={item.isSelectLeft ? { backgroundColor: '#66b2ff' } : {}} onClick={() => { this.props.actions.selectTeachersLeft(item.mid); }} className="teacherButton">{item.name}mid:{item.mid} </span>)
                                            }
                                          </div> : <div className="teachersBeAddedBottom">
                                            {
                                                    state.leftSearch.map((item, idx) => <span key={idx} style={item.isSelectLeft ? { backgroundColor: '#66b2ff' } : {}} onClick={() => { this.props.actions.selectTeachersLeft(item.mid); }} className="teacherButton">{item.name}mid:{item.mid} </span>)
                                                }
                                                   </div>
                                    }
                                  </div>
                              ...
                              <div className="teachersNotAddBottom" />
                                : <div>{
                                    !state.rightIsSearching ?
                                      <div className="teachersNotAddBottom">
                                        {
                                                state.teachersInRight.map((item, idx) => (<span
                                                  key={idx}
                                        ...
                                        {
                                                state.rightSearch.map((item, idx) => <span key={idx} style={item.isSelectLeft ? { backgroundColor: '#66b2ff' } : {}} onClick={() => { this.props.actions.selectTeachersLeft(item.mid); }} className="teacherButton">{item.name}mid:{item.mid} </span>)
                                            }
                                               </div>
                                }
                                  </div>
